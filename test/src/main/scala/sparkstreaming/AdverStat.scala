...
object AdverStat {
  ...
  def getRecentHourClickCount(adRealTimeFilterDStream: DStream[String]) = {
    ...
    val value = adRealTimeFilterDStream.map {
      log =>
        val strings = log.split(" ")
        val long = strings(0).toLong
        val str = DateUtils.formatTimeMinute(new Date(long))
        val adid = strings(4).toLong
        val str1 = str + "_" + adid
        (str1, 1L)
    }
    val value1 = value.reduceByKeyAndWindow((a:Long,b:Long)=> (
